...
export default function Dashboard({ researchData, analysisResults }: DashboardProps) {
  ...
  // Function to extract key insights and recommendations
  const getContentFromAnalysis = (category: string) => {
    const filteredResults = analysisResults.filter((result) => result.category === category);
    return filteredResults.length > 0
      ? filteredResults.map((result, index) => (
          <li key={index} className="text-gray-700 text-sm">{`• ${result.raw_analysis.slice(0, 100)}...`}</li>
        ))
              ...
              {['Hook Ideas', 'CTAs', 'Content Formats'].map((category, index) => (
                <div key={index} className="bg-white bg-opacity-50 p-4 rounded-lg">
                  ...
                  <ul className="space-y-2">
                    {getContentFromAnalysis(category)}
                  </ul>
                </div>
              ))}
            </div>
